...
namespace :secrets do
  ...
  task :share_with, :heroku_app, :env_config do |task, args|
    ...
    @env_config = args[:env_config] || 'production'

    msg  = "This task will sync ALL ENVIRONMENT VARIABLES in the `#{@heroku_app}` app "
    msg += "with the values defined in the `#{@env_config}` group of config/secrets.yml. "
    ...
    sh "heroku config:add #{Secrets.for_heroku(@env_config)} --app #{@heroku_app}"
  end
end
